# Handoff Report: Milestone 1 — Student Exam Hub (`/student/exams` & `/api/student/exams`)

## 1. Observation
- **`src/app/api/student/exams/route.ts`**:
  - Line 9: Authenticates student session using `getLoggedInStudent()`.
  - Lines 45–57: Calculates `isUpcoming = Boolean(q.openAt && now < new Date(q.openAt))` and `isPastClose = Boolean(q.closeAt && now > new Date(q.closeAt))`. Sets `availability: "UPCOMING" | "OPEN" | "CLOSED"`.
  - Lines 68–70: Serializes `openAt: q.openAt ? q.openAt.toISOString() : null`, `closeAt: q.closeAt ? q.closeAt.toISOString() : null`, `availability`.
  - Lines 85–96: Includes `attempt` object (`status`, `score`, `totalScore`, `strikeCount`, `startedAt`, `submittedAt`).
- **`src/app/student/exams/page.tsx`**:
  - Lines 434–439: Contains **3 duplicate closing `</div>` tags** (lines 436, 437, 438) causing JSX syntax parse errors:
    ```tsx
    434:                 </div>
    435:               </div>
    436:                   </div>
    ...
    439:             );
    ```
  - Lines 109–115: `completedCount` calculates `e.attempt?.status === "SUBMITTED" || e.attempt?.status === "GRADED"` but omits `"DISQUALIFIED"`, unlike line 102.
  - Lines 99–106: Filtering for `activeTab === "ACTIVE"` checks `!isDone`. If an exam is `CLOSED` and the student never started it, it currently appears under `ACTIVE` ("Tersedia / Aktif"), which creates UX ambiguity.
  - Lines 388–403: Basic disabled buttons exist for `UPCOMING` and `CLOSED` without dynamic live countdown ticking or contextual scheduling helpers.
  - Mobile layout (`< 640px`): Card action footer (`line 337`) needs responsive flex wrapping (`flex-col sm:flex-row`) to ensure touch-friendly button targets without horizontal overflow.

---

## 2. Logic Chain
1. **API Readiness**:
   - `src/app/api/student/exams/route.ts` already returns `openAt`, `closeAt`, and calculated `availability`. No database schema or structural API route changes are needed; the contract satisfies all Milestone 1 requirements.
2. **Client Syntax Fix**:
   - Removing lines 436–438 from `src/app/student/exams/page.tsx` fixes the stray closing `</div>` tags and restores TypeScript/JSX compilation.
3. **Tab Filtering Refinement**:
   - Categorize exams into:
     - `isDone`: `attempt.status === "SUBMITTED" | "GRADED" | "DISQUALIFIED"`
     - `isExpired`: `exam.availability === "CLOSED" && !exam.attempt`
   - Tab "Tersedia / Aktif" (`ACTIVE`) filters for `!isDone && !isExpired` (exams that are `OPEN` or `UPCOMING` or in-progress).
   - Tab "Riwayat / Selesai" (`COMPLETED`) filters for `isDone || isExpired`.
   - Tab "Semua" (`ALL`) shows all exams.
   - Update tab count badges to match this exact logic so sums are consistent.
4. **Dynamic Countdown & Status Badges**:
   - Introduce a 1-second ticking clock state (`currentTime`) in `StudentExamsPage`.
   - For `UPCOMING` exams:
     - Badge: Amber `Calendar` badge `"Ujian Belum Dibuka"`.
     - Schedule banner: Indonesian locale format `"Buka: DD MMM YYYY, HH:mm WIB"`.
     - If `< 24 hours` until `openAt`: Display realtime countdown pill `"⏱️ Dibuka dlm: HH:mm:ss"`.
     - Action button: Disabled `"Belum Dibuka"` with `Lock` icon.
   - For `OPEN` exams:
     - Badge: Blue/Emerald `"Tersedia / Berlangsung"`.
     - Schedule banner: `"Tutup: DD MMM YYYY, HH:mm WIB"` (if `closeAt` is set). If closing in `< 2 hours`, show warning `"⏱️ Sisa waktu: Xj Ym"`.
     - Action button: Active gradient `"Ikuti Ujian"` / `"Mulai Ujian"`.
   - For `CLOSED` exams:
     - Badge: Slate `"Ujian Telah Ditutup"`.
     - Schedule banner: `"Telah Ditutup pada: DD MMM YYYY, HH:mm WIB"`.
     - Action button: Disabled `"Ujian Ditutup"` with `Clock` icon.
   - For `IN_PROGRESS` / `LOCKED`:
     - Badge: Pulsing amber `"Sedang Dikerjakan"` / red `"Terkunci Pelanggaran"`.
     - Action button: Active `"Lanjutkan Ujian"` / `"Buka Kunci"`.
5. **Mobile Responsiveness**:
   - Ensure header banner, tab pills, search box, card grids, meta info tags, and bottom action bars use `flex-col sm:flex-row`, `overflow-x-auto`, and responsive padding to comply 100% with VeloNet Mobile Responsiveness standard.

---

## 3. Caveats
- **Time Synchronization**: Client-side countdown uses `Date.now()` compared against server UTC ISO timestamps (`openAt`, `closeAt`). If the student's device clock is significantly off, the countdown may differ by a few minutes, but the server `/api/quiz/[quizId]/start` is the authoritative source of truth.
- **Auto-Transition**: When a countdown reaches `00:00:00`, the component will recalculate status on next tick and transition seamlessly without requiring a full page refresh.

---

## 4. Conclusion
The implementation plan for the Student Exam Hub is complete and fully scoped. The implementer agent can directly apply the proposed modifications to `src/app/student/exams/page.tsx` and verify `src/app/api/student/exams/route.ts`.

### Proposed Code for `src/app/student/exams/page.tsx`:
```tsx
"use client";
...
1. **Compilation Check**:
   Run `npm run build` or `npx tsc --noEmit` to confirm 0 TypeScript and JSX syntax errors.
2. **Behavioral Test Cases**:
   - **Case 1 (Upcoming Exam)**: When `openAt` is in the future, card shows "Ujian Belum Dibuka", live ticking countdown "Dibuka dalam: HH:mm:ss", and disabled button "Belum Dibuka".
   - **Case 2 (Open Exam)**: When current time is between `openAt` and `closeAt` (or both null), card shows "Sedang Berlangsung", active "Mulai Ujian" button linking to `/student/quiz/[quizId]`.
   - **Case 3 (Closed Exam)**: When `closeAt` is in the past without an attempt, card shows "Ujian Telah Ditutup" and disabled button "Ujian Ditutup".
   - **Case 4 (In-Progress Exam)**: When student started before `closeAt`, card shows "Sedang Dikerjakan" and active "Lanjutkan Ujian" button.
   - **Case 5 (Completed Exam)**: Shows "Selesai Dikerjakan", score (if visible), and "Lihat Hasil Ujian" + "Peringkat" button.
   - **Case 6 (Mobile Viewport < 640px)**: Verify clean responsive stacking, no table/card overflow, and touch-target sizes >= 40px.
