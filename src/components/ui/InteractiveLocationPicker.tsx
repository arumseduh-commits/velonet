...
export default function InteractiveLocationPicker({
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="relative">
        <form onSubmit={handleSearch} className="relative flex items-center">
          <input
            ...
            className="w-full pl-9 pr-24 py-2 rounded-xl bg-slate-950 border border-slate-700 text-white placeholder-slate-500 text-xs focus:outline-none focus:border-emerald-500 transition-colors shadow-inner"
          />
          ...
          <button
            ...
            className="absolute right-1.5 px-3 py-1 rounded-lg bg-emerald-600 hover:bg-emerald-500 text-white text-[11px] font-bold transition-all disabled:opacity-40 flex items-center gap-1 cursor-pointer"
          >
        ...
        {showSearchResults && searchResults.length > 0 && (
          <div className="absolute z-50 top-full mt-1.5 left-0 right-0 bg-slate-900 border border-slate-700 rounded-xl shadow-2xl overflow-hidden max-h-56 overflow-y-auto divide-y divide-slate-800">
            {searchResults.map((item) => (
              <button
                type="button"
                key={item.place_id}
                onClick={() => handleSelectSearchResult(item)}
                className="w-full p-2.5 text-left hover:bg-emerald-950/40 text-slate-200 hover:text-emerald-300 transition-colors flex items-start gap-2 text-xs cursor-pointer"
              >
      ...
      <div className="flex items-center gap-1.5 overflow-x-auto pb-1 text-[10px]">
        ...
        {QUICK_REGIONS.map((region) => (
          <button
            ...
            className="px-2 py-1 rounded-lg bg-slate-900 hover:bg-emerald-950/60 border border-slate-700 hover:border-emerald-500/40 text-slate-300 hover:text-emerald-300 font-medium whitespace-nowrap transition-all cursor-pointer"
          >
            📍 {region.name}
          </button>
        ))}
      ...
      <div className="relative rounded-2xl overflow-hidden border border-slate-800 bg-slate-950 shadow-2xl">
        ...
        <div className="absolute top-2.5 right-2.5 z-20 flex items-center gap-2">
          <button
            ...
            className="px-2.5 py-1.5 rounded-xl bg-slate-900/90 hover:bg-slate-800 border border-slate-700/80 text-blue-400 hover:text-blue-300 text-[11px] font-bold shadow-lg backdrop-blur-md transition-all flex items-center gap-1.5 cursor-pointer disabled:opacity-50"
        ...
        <div className="absolute bottom-2 inset-x-2 z-20 pointer-events-none">
          <div className="bg-slate-950/85 backdrop-blur-md px-3 py-1.5 rounded-xl border border-slate-800 text-[10px] text-slate-300 flex items-center justify-between shadow-lg">
      ...
      {gpsAccuracyWarning && (
        <div className="p-3 rounded-xl bg-amber-950/40 border border-amber-500/40 text-amber-200 text-xs flex items-start gap-2.5 animate-in fade-in duration-200">
          <AlertTriangle className="w-4 h-4 text-amber-400 shrink-0 mt-0.5" />
          <div className="space-y-1">
            <p className="font-bold text-[11px]">Catatan Deteksi Lokasi Browser:</p>
            <p className="text-[11px] text-amber-300/90 leading-relaxed">{gpsAccuracyWarning}</p>
          </div>
        </div>
      )}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 text-[11px]">
          <div>
            ...
            <input
              ...
              className="w-full px-2.5 py-1.5 rounded-lg bg-slate-900 border border-slate-700 text-white text-[11px] focus:outline-none focus:border-emerald-500 font-mono"
            />
          </div>
          ...
          <div>
            <label className="text-slate-400 block mb-0.5">Radius Toleransi (Meter)</label>
            <select
              value={radiusMeter}
              onChange={(e) => onRadiusChange(e.target.value)}
              className="w-full px-2.5 py-1.5 rounded-lg bg-slate-900 border border-slate-700 text-white text-[11px] focus:outline-none focus:border-emerald-500 font-semibold"
            >
