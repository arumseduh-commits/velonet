...
export default function AdminEditExamPage({
    ...
    fetch(`/api/admin/exams/${quizId}`)
      ...
      .then((json) => {
        if (json.success && json.data) {
          ...
          setEnableCameraProctor(Boolean(q.enableCameraProctor));
          ...
          toast.error("Modul ujian tidak ditemukan.");
          router.push("/admin/exams");
        }
      })
      .catch(() => {
        toast.error("Gagal memuat data modul ujian.");
      })
  ...
  const handleSaveExam = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch(`/api/admin/exams/${quizId}`, {
        method: "PATCH",
      ...
      const json = await res.json();
      if (json.success) {
        toast.success("Modul ujian & butir soal berhasil diperbarui!");
        router.push("/admin/exams");
      } else {
        toast.error(json.error || "Gagal memperbarui modul ujian.");
      }
    } catch (err: any) {
  ...
  if (loading) {
    return (
      <div className="p-20 flex flex-col items-center justify-center text-slate-400 text-xs">
        <RefreshCw className="w-8 h-8 animate-spin text-blue-600 mb-3" />
        <span>Memuat data modul ujian...</span>
      </div>
    );
  }
    ...
    <form onSubmit={handleSaveExam} className="space-y-8 pb-32 max-w-5xl mx-auto">
      ...
      <div className="sticky top-4 z-40 p-4 rounded-3xl bg-slate-900/95 backdrop-blur-md text-white border border-slate-800 shadow-2xl flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="font-extrabold text-sm sm:text-base text-white tracking-tight flex items-center gap-2">
              <span>Edit Modul Ujian & Soal</span>
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <Link
            ...
            target="_blank"
            className="px-3 py-2 rounded-xl bg-white/10 hover:bg-white/20 text-slate-200 text-xs font-semibold transition-colors flex items-center gap-1.5"
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div>
            ...
            <p className="text-xs text-slate-500">
              Kelola butir soal dengan dukungan 5 tipe soal & gambar terkompresi WebP.
            </p>
          </div>
      ...
      <div className="p-4 rounded-3xl bg-white border border-slate-200 shadow-md flex items-center justify-between gap-4">
        ...
        >
          ...
          <span>Simpan Perubahan Modul Ujian</span>
        </button>
      </div>
    </form>
  );
